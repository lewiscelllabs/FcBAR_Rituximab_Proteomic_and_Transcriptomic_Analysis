...
# Select the top 7 genes based on absolute fold change and  padj-values (use this instead of pval to highlight some of the validated genes) to highlight on the volcano plot
top_genes <- results %>%
  filter(padj < significance_threshold) %>%  # Keep only significant genes
  arrange(desc(abs(log2_Fold_Change)), padj) %>%  # Sort by highest fold change, then lowest padj
  slice_head(n = 7)  # Select top 7 genes

# Create the volcano plot using p-values
